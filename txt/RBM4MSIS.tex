...
The AHC  algorithm requires specifying a linkage criterion, which determines how clusters are merged based on the distance between them. Standard linkage methods include single linkage, complete linkage, and average linkage. This work will use the complete linkage and the average linkage criterion to measure the distances between clusters during the merging process. Average linkage defines the distance between two clusters as the average of all pairwise distances between points in the two clusters \cite{Müllner2011, Bratchell1989}. Formally, for two clusters $C_i$ and $C_j$, the average linkage distance is given by the formula:

\begin{equation}\label{avg_link}
    d_{\text{avg}}(C_i, C_j) = \frac{1}{|C_i||C_j|} \sum_{x \in C_i} \sum_{y \in C_j} d(x, y),
\end{equation}

\noindent where \( |C_i| \) and \( |C_j| \) are the number of points in clusters \( C_i \) and \( C_j \), respectively, and \( d(x, y) \) represents the distance between points \( x \) and \( y \).

Complete linkage defines the distance between two clusters as the maximum distance between any pair of points in the two clusters \cite{Müllner2011}. Formally, for two clusters \( C_i \) and \( C_j \), the complete linkage distance is given by the formula:

\begin{equation}\label{complete_link}
    d_{\text{complete}}(C_i, C_j) = \max_{x \in C_i, \, y \in C_j} d(x, y),
\end{equation}

\noindent This criterion tends to produce compact clusters by prioritizing the maximum distance, which discourages merging clusters that are widely spread.

The AHC algorithm works on pre-clustered data. In Chapter \ref{methodology}, it will be described why exactly we will use AHC.
